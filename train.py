...
from data_loader import BSDSLoader, faultsDataset
from HED_org import HED
from functions import sigmoid_cross_entropy_loss, cross_entropy_loss
...
# ================ dataset
parser.add_argument('--dataset', help='root folder of dataset', default='C:\hed\hed\data\HED-BSDS')
...
def main():
    args.cuda = True
    # dataset
    train_dataset = faultsDataset(imgs_dir="D:\CNNforFaultInterpretation\data_AN\patched_data\\train\seismic",
                                  masks_dir="D:\CNNforFaultInterpretation\data_AN\patched_data\\train\\annotation")
    test_dataset = faultsDataset(imgs_dir="D:\CNNforFaultInterpretation\data_AN\patched_data\\test\seismic",
                                  masks_dir="D:\CNNforFaultInterpretation\data_AN\patched_data\\test\\annotation")
...
def test(model, test_loader, epoch, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        result_b = Image.fromarray(((1 - result) * 96).astype(np.uint8))
        result = Image.fromarray((result * 96).astype(np.uint8))
